    ...
    while True:
        ...
        if parser.dirty:
            ...
            try:
                ...
                # A transient read of the live parser dicts (mutated on the controller reader thread)
                # must never kill this loop — otherwise the web AP/Station tables silently stop
                # updating for the rest of the session. Mark dirty so the next tick retries.
                parser.dirty = True
