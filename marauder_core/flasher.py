...
def esptool_argv(*args: str) -> List[str]:
    ...
    Source / pip installs run under a real Python interpreter, so `python -m esptool` works. But in a
    PyInstaller build `sys.executable` is THIS app's exe (not a Python), so `-m esptool` would silently
    re-launch the GUI and esptool would never run — breaking esptool_available(), _detect_chip(),
    flash(), erase() and flash_suicide() in every shipped binary. When frozen we instead re-exec the app
    exe with a sentinel that run_esptool_entrypoint() intercepts at startup to run esptool's own CLI
    in-process, which keeps _run_stream's line-streaming subprocess model intact.
    """
...
def run_esptool_entrypoint(argv: Optional[List[str]] = None) -> bool:
    ...
    `_ESPTOOL_SENTINEL` as argv[1]. This must be called FIRST in the frozen entrypoint (before the GUI
    is imported): if the sentinel is present it hands the remaining args to esptool's own CLI
    (`esptool._main`, matching `python -m esptool` error handling / exit codes) and never returns — it
    raises SystemExit with esptool's status. If the sentinel is absent it is a no-op and returns False
    so the caller proceeds to launch normally.
    """
    argv = list(sys.argv if argv is None else argv)
    if len(argv) < 2 or argv[1] != _ESPTOOL_SENTINEL:
        return False
    import esptool
    # Present esptool a normal argv (drop our program name + sentinel) and hand off to its CLI entry so
    # behavior matches `python -m esptool` exactly (FatalError -> exit 2, etc.).
