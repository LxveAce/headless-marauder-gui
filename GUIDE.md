...
Recon feeds targeting, targeting feeds attacks, and the captures feed into other tools.
...
- **WiGLE** for wardrive CSVs (chain D).
...
| **ESP32-DIV** ([cifertech/esp32-div](https://github.com/cifertech/esp32-div)) | A separate standalone ESP32 firmware. Flash-only here: once it's on the board it runs on its own, and this app has no native control panel for it. | Fetches the official ESP32-DIV image and its boot chain and flashes them byte-for-byte. | No. |
...
4. **FLASH** writes every offset/image pair from the manifest in one
   `write_flash -z --flash_size detect`. It burns no eFuses and performs no T2 here.
...
- **T2 / flash encryption is irreversible.** If the bundle was provisioned to burn T2
  (flash-encryption eFuses), that is a one-way, permanent change to the chip that cannot be
...
For authorized security testing only: networks and devices you own or have written permission
